    ...
    companion object {
        ...
        fun getRequests(token: String, success: (requestList: List<myapp.com.karry.entity.Request>) -> Unit, failure: () -> Unit) {
            ...
            OkHttpClient().newCall(request).enqueue(object : Callback {
                override fun onResponse(call: Call, response: Response) {
                    if (response.code() == 200) {
                        success(Gson().fromJson(response.body()!!.string(), Array<myapp.com.karry.entity.Request>::class.java).toList())
                    } else {
                        failure()
                    }
                }
        ...
        fun getRooms(token: String, success: (roomList: List<Room>) -> Unit, failure: () -> Unit) {
            ...
            OkHttpClient().newCall(request).enqueue(object : Callback {
                override fun onResponse(call: Call, response: Response) {
                    if (response.code() == 200) {
                        success(Gson().fromJson(response.body()!!.string(), Array<Room>::class.java).toList())
                    } else {
                        failure()
                    }
                }
